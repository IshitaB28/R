...
my_net<-read.paj("F:\\Stat202\\Comments_uuid50_50.net")
class(my_net)
...
plot(my_net,displaylabels=T,boxed.labels=F)
summary(my_net)
...
write_graph(g2, "F:\\Stat202\\Openstack_Comments_Network.net", "pajek")

#fitting the ERGM

library(ergm)
all_on_edge_comments<-ergm(my_net_r~edges+nodecov("eigen_c")+nodecov("degree")+nodecov("betweenness")+nodecov("closeness")+nodecov("pagerank")+nodecov("clustcoeff"))
...
my_net2<-read.paj("F:\\Stat202\\Changes_uuid50_50.net")
class(my_net2)
...
write_graph(gx2, "F:\\Stat202\\Openstack_Changes_Network.net", "pajek")
...
class(vert_name_com)
vert_name_com
open_com_mets <- data.frame(vert_name_com, degree, betweenness, closeness, clust_coeff, pagerank, eigen_c)
write.csv(open_com_mets, "F:\\Stat202\\OpenStack_Comments_Metrics.csv")
write.csv(open_com_mets,"C:\\Users\\Ishita Bardhan\\Dropbox\\HIT-2020-4th-year\\02-research-journals\\ishita\\01-code\\OpenStack_Comments_Metrics.csv")


vert_name_chn<-V(gx2)
vert_name_chn<-as_ids(vert_name_chn)
class(vert_name_chn)
open_chn_mets <- data.frame(vert_name_chn, degree2, betweenness2, closeness2, clust_coeff2, pagerank2, eigen_c2)
write.csv(open_chn_mets, "F:\\Stat202\\OpenStack_Changes_Metrics.csv")
write.csv(open_chn_mets,"C:\\Users\\Ishita Bardhan\\Dropbox\\HIT-2020-4th-year\\02-research-journals\\ishita\\01-code\\OpenStack_Changes_Metrics.csv")
